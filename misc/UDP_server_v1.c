...
HANDLE worker_ack_thread[MAX_CLIENTS];
    ...
    if (!client) {
        if (received_frame_type == FRAME_TYPE_CONNECT_REQUEST) {
            ...
            if (client) {
                ...
                push_queue(&client->queue_ack, received_seq_num, &client->queue_event);
        ...
        // Optionally, update client info if needed (e.g., flags, name)
        if (received_frame_type == FRAME_TYPE_CONNECT_REQUEST) {
            // If a connect request is received again, update the client info
            client->flags = frame->payload_data.request.flags;
            strncpy(client->client_name, frame->payload_data.request.client_name,
                    sizeof(client->client_name) - 1);
            client->client_name[sizeof(client->client_name) - 1] = '\0'; // Ensure null termination
        }
    }
...
unsigned int WINAPI send_ack_thread_func(LPVOID lpParam){
    ...
    while(running) {
        ...
        printf("Event received for client:\n");
            if(client->queue_ack.tail == client->queue_ack.head){
                continue;
            }
            //if queue is not empty then send acknowledge for frame sequence number
            int ack_seq_index = client->queue_ack.head;
            send_ack_nack(FRAME_TYPE_ACK, client->queue_ack.buffer[ack_seq_index], server_socket, &client->addr);
...
ClientInfo* add_client(const UdpFrame *recv_frame, const struct sockaddr_in *addr) {
    ...
    worker_ack_thread[num_connected_clients] = (HANDLE)_beginthreadex(NULL, 0, send_ack_thread_func, new_client, 0, NULL);
    printf("Starting ack thread: %d\n", num_connected_clients);
