...
err_code_t send_packet_in_frames(SOCKET clientSocket, const void* packet) {
    ...
    while (bytes_remaining > 0) {
        ...
        // Calculate the total size of the frame structure in host byte order
        ...
        // Compute CRC-32 for the payload
        // Note: crc32 function expects payload in host byte order, so we pass the raw payload       
        frame.CRC = htonl(crc32(frame.payload, current_frame_payload_size));
        
        bytes_remaining -= current_frame_payload_size;
        current_offset += current_frame_payload_size;
        // Send the frame over the network
        // int bytes_sent = send(clientSocket, (char*)&frame, current_frame_payload_size, 0);
        // if (bytes_sent == SOCKET_ERROR) {
        //  log_event(LOG_ERROR, "Failed to send frame."); // Add specific error logging
