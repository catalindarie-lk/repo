...
#define CLIENT_TIMEOUT_SEC  30          // Seconds after which an inactive client is considered disconnected
...
typedef struct {
    ...
    uint32_t sequence_num;       // Global sequence number for this frame from the sender
...
typedef struct {
    ...
    uint32_t client_id; // Unique ID assigned by server
...
void send_ack(const struct sockaddr_in *dest_addr, uint32_t ack_seq_num, uint32_t target_id, uint32_t target_offset);
...
ClientInfo* add_client(const struct sockaddr_in *addr);
...
void send_ack(const struct sockaddr_in *dest_addr, uint32_t ack_seq_num, uint32_t target_id, uint32_t target_offset) {
    ...
    ack_frame.header.sequence_num = htonl(server_out_seq++);
    ack_frame.payload_data.ack_nack.acknowledged_seq_num = htonl(ack_seq_num);
    ack_frame.payload_data.ack_nack.target_id = htonl(target_id);
    ack_frame.payload_data.ack_nack.target_offset = htonl(target_offset);
    ...
    send_frame(&ack_frame, dest_addr);
    char dest_ip[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &(dest_addr->sin_addr), dest_ip, INET_ADDRSTRLEN);
    printf("Sending ACK for Seq %u to %s:%d (Target ID: %u, Offset: %u)\n",
           ack_seq_num, dest_ip, ntohs(dest_addr->sin_port), target_id, target_offset);
}
...
void send_nack(const struct sockaddr_in *dest_addr, uint32_t nack_seq_num, uint32_t target_id, uint32_t target_offset) {
    ...
    nack_frame.header.sequence_num = htonl(server_out_seq_nack++);
    nack_frame.payload_data.ack_nack.acknowledged_seq_num = htonl(nack_seq_num);
...
void send_file_transfer_status(const struct sockaddr_in *dest_addr, uint32_t file_id, const uint8_t *final_hash, BOOL success) {
    ...
    status_frame.header.sequence_num = htonl(status_seq++);
...
void process_received_frame(UdpFrame *frame, int bytes_received, const struct sockaddr_in *sender_addr) {
    ...
    uint32_t received_seq_num = ntohl(frame->header.sequence_num);
    // uint32_t received_ack_num = ntohl(frame->header.ack_num); // Not directly used in current processing
    ...
    if (!client) {
        if (received_frame_type == FRAME_TYPE_CONNECT_REQUEST) {
            client = add_client(sender_addr);
            if (client) {
                printf("New client connected from %s:%d (ID: %u)\n", sender_ip, sender_port, client->client_id);
                // Send CONNECT_RESPONSE
                UdpFrame resp_frame;
                memset(&resp_frame, 0, sizeof(resp_frame));
                resp_frame.header.start_delimiter = htons(FRAME_DELIMITER);
                resp_frame.header.frame_type = FRAME_TYPE_CONNECT_RESPONSE;
                static uint32_t connect_resp_seq = 0;
                resp_frame.header.sequence_num = htonl(connect_resp_seq++);
                // Client ID can be sent in a payload if needed, but for now, it's server-side
                resp_frame.header.checksum = htonl(calculate_crc32(&resp_frame, sizeof(FrameHeader)));
                send_frame(&resp_frame, sender_addr);
                send_ack(sender_addr, received_seq_num, 0, 0); // ACK the connect request
            } else {
        ...
        if (received_seq_num <= client->last_received_seq) {
            ...
            send_ack(sender_addr, received_seq_num, 0, 0); // Re-ACK duplicates
            LeaveCriticalSection(&client_list_mutex);
            return;
        }
        client->last_received_seq = received_seq_num; // Update last received sequence
    }
    LeaveCriticalSection(&client_list_mutex); // Release mutex after client operations
    ...
    switch (received_frame_type) {
        case FRAME_TYPE_TEXT_MESSAGE: {
            ...
            send_ack(sender_addr, received_seq_num, 0, 0); // Acknowledge receipt
        ...
        case FRAME_TYPE_LONG_TEXT_MESSAGE_DATA: {
            ...
            send_ack(sender_addr, received_seq_num, msg_id, offset);
            break;
        }
        case FRAME_TYPE_FILE_METADATA_REQUEST: {
            ...
            if (ctx) {
                ...
                resp_frame.header.sequence_num = htonl(file_meta_resp_seq++);
                ...
                send_ack(sender_addr, received_seq_num, 0, 0); // ACK the request
            } else {
                ...
                if (ctx->temp_file_handle) {
                    ...
                    if (fseek(ctx->temp_file_handle, offset, SEEK_SET) == 0) {
                        ...
                        if (bytes_written == payload_len) {
                            ...
                            send_ack(sender_addr, received_seq_num, file_id, offset); // ACK this specific fragment
        ...
        case FRAME_TYPE_DISCONNECT: {
            ...
            send_ack(sender_addr, received_seq_num, 0, 0); // ACK the disconnect
            break;
        }
        case FRAME_TYPE_KEEP_ALIVE: {
            ...
            send_ack(sender_addr, received_seq_num, 0, 0); // Acknowledge keep-alive
            break;
        }
        case FRAME_TYPE_CONNECT_REQUEST:
...
ClientInfo* find_client(const struct sockaddr_in *addr) {
    ...
    for (int i = 0; i < num_connected_clients; i++) {
        if (connected_clients[i].address.sin_addr.s_addr == addr->sin_addr.s_addr &&
            connected_clients[i].address.sin_port == addr->sin_port) {
...
ClientInfo* add_client(const struct sockaddr_in *addr) {
    ...
    memcpy(&new_client->address, addr, sizeof(struct sockaddr_in));
    new_client->last_activity_time = time(NULL);
    new_client->client_id = num_connected_clients + 1; // Simple ID assignment
...
void manage_clients_and_transfers() {
    ...
    for (int i = 0; i < num_connected_clients; /* no increment here */) {
        if (current_time - connected_clients[i].last_activity_time > CLIENT_TIMEOUT_SEC) {
            char ip_str[INET_ADDRSTRLEN];
            inet_ntop(AF_INET, &(connected_clients[i].address.sin_addr), ip_str, INET_ADDRSTRLEN);
            printf("Client %s:%d timed out (inactive for %d seconds). Disconnecting.\n",
                   ip_str, ntohs(connected_clients[i].address.sin_port), CLIENT_TIMEOUT_SEC);
            remove_client(&connected_clients[i].address); // This will decrement num_connected_clients and adjust 'i'
        } else {
