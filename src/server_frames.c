...
int construct_ack_frame(PoolEntrySendFrame *entry,
                    const uint64_t seq_num, 
                    const uint32_t session_id, 
    ...
    frame->payload.ack.file_id = _htonl(file_id);
    frame->payload.ack.op_code = op_code;
    frame->header.checksum = _htonl(calculate_crc32(frame, sizeof(FrameHeader) + sizeof(AckPayload)));
...
int construct_file_metadata_response_frame(PoolEntrySendFrame *entry,
                    const uint64_t seq_num, 
                    const uint32_t session_id,
                    const uint32_t file_id, 
                    const uint8_t op_code, 
...
int construct_file_end_response_frame(PoolEntrySendFrame *entry,
                    const uint64_t seq_num, 
                    const uint32_t session_id,
                    const uint32_t file_id, 
                    const uint8_t op_code, 
    ...
    frame->header.start_delimiter = _htons(FRAME_DELIMITER);
    frame->header.frame_type = FRAME_TYPE_FILE_END_RESPONSE;
    frame->header.seq_num = _htonll(seq_num);
    frame->header.session_id = _htonl(session_id);
    frame->payload.file_end_response.file_id = _htonl(file_id);
    frame->payload.file_end_response.op_code = op_code;
    frame->header.checksum = _htonl(calculate_crc32(frame, sizeof(FrameHeader) + sizeof(FileEndResponsePayload)));
